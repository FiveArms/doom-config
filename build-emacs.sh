...
## This script builds GNU Emacs 30 with support for
## native elisp compilation, tree-sitter, and mailutils.
...
# Lots of cruft from unavailable or old packages removed...
# See the original list if encountering errors or missing dependencies
sudo apt install -y autoconf automake bsd-mailx build-essential \
  ...
  git gvfs ibus-gtk3 language-pack-en-base libacl1-dev \
  mailutils procps quilt sharutils texinfo zlib1g-dev

# Issues with libwebkit2gtk
# possible fix from https://unix.stackexchange.com/a/784289
...
#    --with-pgtk                ->  NOT useful with WSLg; better font rendering
#    --with-x-toolkit=gtk3      ->  widgets toolkit
#    --with-tree-sitter         ->  syntax parsing
#    --with-wide-int            ->  larger file size limit
#    --with-xwidgets            ->  use xwidgets in Emacs buffers (requires gtk3 or macOS Cocoa)
#    --with-gnutls              ->  TLS/SSL
...
./autogen.sh \
    ...
    #--with-x-toolkit=lucid         # supposedly more stable; older/worse than gtk3
...
# Build and install emacs.
# NOTE: NATIVE_FULL_AOT=1 ensures native compilation ahead-of-time for all
